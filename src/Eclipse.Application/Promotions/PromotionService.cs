...
internal sealed class PromotionService : IPromotionService
{
    ...
    {
        var promotion = Promotion.Create(request.Title, request.FromChatId, request.MessageId, request.InlineButtonText, request.InlineButtonLink, _timeProvider.Now);
    ...
    public async Task<Result<PromotionDto>> Update(Guid id, UpdatePromotionRequest request, CancellationToken cancellationToken = default)
    {
        var promotion = await _promotionsRepository.FindAsync(id, cancellationToken);
