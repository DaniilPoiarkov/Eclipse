...
public sealed class CurrentCultureTests
{
    ...
    private IStringLocalizer<CurrentCultureTests> Localizer => _serviceProvider.GetRequiredService<IStringLocalizer<CurrentCultureTests>>();
    ...
    [InlineData("Test3", "uk", "Test 3", "Тест 3")]
    [InlineData("Test4", "uk", "Test 4", "Тест 4")]
    [InlineData("ExceptionMessage", "uk", "Exception {0}", "Помилка {0}")]
    public void UsingCulture_WhenSpecified_ThenUsesSpecificCultureInScope(string key, string culture, string expectedWithDefault, string expectedWithUsing)
    {
        var testBeforeUsing = Localizer[key];

        var testWithUsing = new LocalizedString(string.Empty, string.Empty);

        using (var _ = Sut.UsingCulture(culture))
        {
            testWithUsing = Localizer[key];
        }

        var testAfterUsing = Localizer[key];
